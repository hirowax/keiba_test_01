...
| ユーザーの指示 | 内容 | 前提条件 |
|---|---|---|
| 「Phase Aの確認やって」 | オッズ保存改修（実装済み 2026-07-06）の初回開催日チェック | 次の開催日にrun.sh実行済み |
| 「柱3やって」 | JRA公式払戻スクレイパー実装 + 3連複の事前登録検証 | なし |
| 「Phase Bやって」 | スコア確率化 + EVフィルターのバックテスト | クリーンデータ90日以上（2026-11頃） |
| 「Phase Cやって」 | 損益分岐オッズ表示のUI実装 | Phase Bが採用基準クリア |

- Phase A実装により `pickup_scores.json` に `odds_map`（レース単位）と `today_odds`（scored各馬）が保存される（2026-07-11以降）
- 同時に人気取得のバグ修正済み（オッズセル誤取得→`Popular_Ninki`厳密判定。today_popの欠損率が改善）
